...
import { selecCartItems, selectCartState, selectTotalAmount, selectTotalQTY, setClearCartItems, setCloseCart, setGetTotals } from '../app/CartSlice'
    ...
    <>
      <div className={`fixed top-0 left-0 right-0 bottom-0 blur-effect-theme w-full h-screen opacity-100 z-[250] ${ifCartState ? "opacity-100 visible translate-x-0" : "opacity-0 invisible translate-x-8"}`}>
        <div className={`blur-effect-theme h-screen max-w-xl w-full absolute right-0`}>
            <CartCount totalQty={totalQty} onCartTouggle={onCartTouggle} onClearCartItems={onClearCartItems}/>

            {cartItems?.length === 0 ? <CartEmpty onCartTouggle={onCartTouggle}/> : <div>
                ...
                <div className='fixed bottom-0 bg-white w-full px-5 py-2 grid items-center'>
                    ...
                    <div className='grid items-center gap-2'>
                        <p className='test-sm font-medium text-center'>Taxes and Shipping will Calculate At Shipping</p>
