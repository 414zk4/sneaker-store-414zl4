        ...
        <header className={
            ...
            <nav className='flex items-center justify-between nike-container'>
                ...
                <ul className='flex items-center justify-center gap-2'>
                    ...
                    <li className={`grid items-center `}>
                        <button type='button' onClick={onCartTouggle} className='broder-none outline-none active:scale-110 transition-all duration-300 relative'>
                            <ShoppingBagIcon className={`icon-style ${navState && "text-slate-900 transition-all duration-300"}`}/>
                            <div className={`absolute top-4 right-0 shadow w-4 h-4 text-[0.65rem] leanding-tight font-medium rounded-full flex items-center justify-center cursor-pointer hover:scale-110 transition-all duration-300 ${navState ? 'bg-slate-900 text-slate-100 shadow-slate-900' : 'bg-slate-100 text-slate-900 shadow-slate-100'}`}>{totalQty}</div>
                        </button>
                    </li>
                </ul>
            </nav>
